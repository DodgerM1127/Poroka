...
export default function RSVPFormPovabljeni({ onSuccess }) {
  ...
  async function submit(e) {
    ...
    try {
      ...
      console.log('Submitting RSVP payload (povabljeni):', payload)
      const res = await fetch('/api/rsvp_povabljeni', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
  ...
  return (
    <form onSubmit={submit} className="space-y-4 max-w-md">
      ...
      <div>
        <label className="block text-sm font-medium">Napiši svojo najljubšo pesem za ples oziroma glasbono željo</label>
